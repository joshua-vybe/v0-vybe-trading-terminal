...
interface Strategy {
  ...
  positions: Position[] // Array of positions instead of single position
  totalPnl: number
...
function PositionRow({ position }: { position: Position }) {
  ...
  return (
    <div
      className={`flex items-center gap-1 py-0.5 px-1 text-[8px] font-mono ${
        isDanger ? "bg-[#f59e0b15] animate-pulse" : "bg-[#ffffff05]"
      }`}
    >
      {/* Asset & Direction */}
      ...
      <div
        className={`w-10 text-right ${
          isDanger
            ? "text-[#f59e0b] font-bold animate-pulse"
            : position.liquidationDistance < 15
              ? "text-[#ef4444]"
              : "text-[#ffffff40]"
        }`}
      >
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onToggle}
            className={`px-1.5 py-0.5 text-[8px] border transition-all ${
              isLive
                ? "border-[#22c55e50] text-[#22c55e] hover:bg-[#22c55e20]"
                : "border-[#ffffff30] text-[#ffffff50] hover:border-[#00ffff50] hover:text-[#00ffff]"
            }`}
          >
            {isLive ? "[ ON ]" : "[ OFF ]"}
          </button>
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            {mostDangerousPos && mostDangerousPos.liquidationDistance < 15 && (
              <span
                className={`text-[8px] ${
                  mostDangerousPos.liquidationDistance < 5 ? "text-[#f59e0b] animate-pulse font-bold" : "text-[#ef4444]"
                }`}
              >
              ...
              {strategy.positions.map((pos, idx) => (
                <span
                  ...
                  className={`text-[8px] px-1 py-0.5 rounded ${
                    pos.direction === "long" ? "bg-[#22c55e20] text-[#22c55e]" : "bg-[#ef444420] text-[#ef4444]"
                  } ${pos.liquidationDistance < 5 ? "animate-pulse border border-[#f59e0b]" : ""}`}
                >
                  {pos.direction === "long" ? "▲" : "▼"}
                  {pos.asset}
                </span>
...
export function StrategyControlPanel() {
      ...
      setStrategies((prev) =>
        prev.map((s) => {
          ...
          return {
            ...
            totalPnl,
            totalPnlPercent: totalPnl / 100, // Simplified
            status: newStatus,
          }
        }),
      )
  ...
  return (
    <div className="neon-border glass-panel p-2 h-[260px] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between mb-2 pb-1 border-b border-[#ffffff10]">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-[#00ffff60]">STRATEGY CONTROL</span>
